...
public class InventoryService {
    ...
    public ProductResponse reserveStock(Long productId, StockReservationRequest request) {
        ...
        try {
            Product updated = productRepository.save(product);
            logStockChange(productId, "RESERVE", request.getQuantity(), before, updated.getStockQuantity(), request.getOrderId());
            return mapToResponse(updated);
    ...
    public ProductResponse releaseStock(Long productId, StockReservationRequest request) {
        ...
        try {
            Product updated = productRepository.save(product);
            logStockChange(productId, "RELEASE", request.getQuantity(), before, updated.getStockQuantity(), request.getOrderId());
            return mapToResponse(updated);
        } catch (OptimisticLockException e) {
            throw new StockConcurrencyException("Stock modified by another request");
        }
    ...
    private void logStockChange(Long productId, String type, Integer qty, Integer before, Integer after, Long orderId) {
        ...
        for (ProductStockUpdate update : request.getUpdates()) {
            try {
                Product product = productRepository.findById(update.getProductId())
                        .orElseThrow(() -> new ProductNotFoundException(PRODUCT_NOT_FOUND + ": " + update.getProductId()));
                ...
                logStockChange(product.getId(), "BULK_UPDATE", Math.abs(diff), stockBefore, update.getNewQuantity(), null);
